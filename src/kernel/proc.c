...
int set_static_priority(int priority, int pid)
{
  int old_priority = 0;
  struct proc *p = proc;
  while (p < &proc[NPROC])
...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    {
      acquire(&p->lock);
      if (p->in_queue && p->curr_wait_time > MAXWAITTIME && p->queue_position > 0)
      {
        remove_queue(p, p->queue_position);
        push_queue(p, p->queue_position - 1);
      }
      release(&p->lock);
    }
    ...
    int qpos = 0;
    while (qpos < 5 && found==0)
    {
      while (queue_struct.size[qpos] > 0)
      {
        bestproc = pop_queue(qpos);
        found = 1;
        break;
      }
      qpos++;
  ...
  for (;;)
  {
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      ...
      if (minproc!=0)
      {
        if (DP == min_priority)
        {
          if (minproc->sched_ct == p->sched_ct && p->ctime < minproc->ctime)
          {
              release(&minproc->lock);
              minproc = p;
          }
          else if(minproc->sched_ct == p->sched_ct)
          {
              release(&p->lock);
          }
          else
